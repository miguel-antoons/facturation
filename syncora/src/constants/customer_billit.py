...
class CustomerBillit(BaseModel):
    ...
    VATNumber: str | None = Field(
        default=None, validation_alias="vat_number", frozen=True
    )
    ...
    ContactFirstName: str | None = Field(
        default=None, validation_alias="surname", frozen=True
    )
    ContactLastName: str | None = Field(
        default=None, validation_alias="name", frozen=True
    )
    ...
    @computed_field
    def Addresses(self) -> list[dict[str, str]]:  # noqa: N802
        return [
            {
    ...
    @classmethod
    def extract_single_email(cls, emails: Any) -> Any:  # noqa: ANN401
    ...
    @classmethod
    def extract_single_phone(cls, phones: Any) -> Any:  # noqa: ANN401
    ...
    @classmethod
    def extract_single_mobile(cls, mobiles: Any) -> Any:  # noqa: ANN401
    ...
    def ensure_at_least_one_name(self) -> CustomerBillit:
        if not (self.Name or self.ContactFirstName or self.ContactLastName):
            raise SyncoraError(
                "At least one of Name, ContactFirstName or ContactLastName "
                "must be provided",
                900,
                Severity.HIGH,
            )
