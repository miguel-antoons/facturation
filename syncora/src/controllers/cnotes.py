...
from constants.all import ResponseMessage, RESPONSE_ERROR, RESPONSE_WARNING, RESPONSE_SUCCESS
from constants.customer_back import CUSTOMER_DB_NAME, CUSTOMER_DB_FIRSTNAME, CUSTOMER_DB_COMPANY
from constants.order_back import PEPPOL_DELIVERY_STATUS_PENDING, \
    PEPPOL_DELIVERY_STATUS_UNKNOWN, OrderBack, PEPPOL_DELIVERY_STATUS_SENT, ORDER_BACK_ORDER_NUMBER
...
def get_cnotes() -> Response:
    ...
    for cnote in cnotes:
        ...
        customer_name = f"{customers[cust_id].name or ""} {customers[cust_id].surname or ""}".strip()
...
def pre_peppol_checks(order_data: OrderBack) -> ResponseMessage | None:
    ...
    elif not BillModel.get({ORDER_BACK_ORDER_NUMBER: order_data.aboutInvoiceNumber})[0].peppolDeliveryStatus == PEPPOL_DELIVERY_STATUS_SENT:
        message = f"La facture associé à la note de crédit avec l'ID {order_data.orderNumber} n'a pas encore été envoyée à Peppol. Veuillez d'abord l'envoyer à Peppol avant d'envoyer la note de crédit."
...
def send_cnote_peppol(cnote_id: str) -> Response:
    ...
    if response.status_code in [200, 201]:
        CnoteModel.set_peppol_status(order_data.externalId, PEPPOL_DELIVERY_STATUS_UNKNOWN)
...
def pre_billit_checks(order_data: OrderBack) -> ResponseMessage | None:
    ...
    if order_data.locked:
        ...
        message = f"La note de crédit avec l'ID {order_data.orderId} n'a pas de numéro de note de crédit. Veuillez ajouter un numéro de note de crédit avant de l'envoyer à Billit."
        ...
        message = f"La note de crédit avec l'ID {order_data.orderNumber} fait référence à une facture qui n'a pas encore été envoyée à Billit. Veuillez d'abord envoyer la facture associée à Billit avant d'envoyer la note de crédit."
        ...
        for line in order_data.orderLines:
            if not line.description:
                ...
                break
            elif not line.quantity or line.quantity <= 0:
                ...
                break
            elif not line.unitPriceExcl or line.unitPriceExcl < 0:
...
def send_cnote_billit(bill_id: str) -> Response:
    ...
    return billit.send_billit(
        ...
        callback=lambda response: CnoteModel.set_external_id(bill_id, int(response.json())),
    )
