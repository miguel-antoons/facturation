...
from constants.all import ResponseMessage, RESPONSE_SUCCESS
from constants.customer_back import CUSTOMER_DB_ID, CUSTOMER_DB_NAME, CUSTOMER_DB_FIRSTNAME, CUSTOMER_DB_COMPANY, \
    CUSTOMER_DB_COMMENT, CUSTOMER_DB_POSTAL_CODE, CUSTOMER_DB_CITY, CUSTOMER_DB_VAT_NUMBER, CustomerFront, CustomerBack
...
def create_customer(json: CustomerFront):
    ...
    if not new_customer.name and not new_customer.surname and not new_customer.company:
        raise SyncoraError("Missing required fields: name, surname, or company is required.", 1000)
