...
from constants.all import RESPONSE_ERROR, ResponseMessage, RESPONSE_WARNING, RESPONSE_SUCCESS
...
from constants.order_back import PEPPOL_DELIVERY_STATUS_PENDING, \
    PEPPOL_DELIVERY_STATUS_UNKNOWN, OrderBack, PEPPOL_DELIVERY_STATUS_SENT
from constants.order_front import OrderFront, OrderFrontShort
from controllers import billit
from controllers.billit import send_peppol, send_billit
...
def send_bill_peppol(bill_id: str) -> Response:
    ...
    if response.status_code in [200, 201]:
        BillModel.set_peppol_status(order_data.externalId, PEPPOL_DELIVERY_STATUS_UNKNOWN)
...
def pre_billit_checks(order_data: OrderBack) -> ResponseMessage | None:
    ...
    if order_data.locked:
        ...
        message = f"Facture avec l'ID {order_data.orderId} n'a pas de numéro de facture. Veuillez ajouter un numéro de facture avant de l'envoyer à Billit."
...
def send_bill_billit(bill_id: str) -> Response:
    ...
    return send_billit(
        ...
        pdf_bytes,
        customer_data,
        callback=lambda response: BillModel.set_external_id(bill_id, int(response.json())),
    )
